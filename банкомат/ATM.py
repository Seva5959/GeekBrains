...
def start_to_csv(file_name): # создает csv файл и записывает в него шапку
    ...
    if not file_name in data:
        hedears = ["name", "surename", 'patronymik','telephone','email','password', "number cart", 'CVC2', 'pin code']
...
class ATM:
    ...
    def registration(self):
        ...
        while cheak_number(user_telephone):
            print("Ошибка!")
            user_telephone = input("Введите ваш номер телефона : +7 ").split()
        lam_f = lambda suf,data : suf + ''.join(item for row in data for item in row) #превращаю user_telephone список в строку
